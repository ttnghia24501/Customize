...
{% endfor %}';
    ...
    public function getProductsData($feed, $productAttributes = [], $productIds = [])
    {
        ...
        foreach ($productCollection as $product) {
            ...
            $rule           = $this->createObject(\Magento\CatalogRule\Model\RuleFactory::class)->create();
            $catalogPrice = $rule->calcProductPriceRule($oriProduct,$oriProduct->getPrice());
            $catalogPrice = $this->convertPrice($catalogPrice, $storeId);
            ...
            $product->setData('catalog_price', $catalogPrice);
    ...
    public function getReviewCollection($feed)
    {
        ...
        foreach ($collection as $review) {
            ...
            $product->setUrl($this->getProductUrl($product, $storeId) . $this->getCampaignUrl($feed));
    ...
    public function deliveryFeed($feed)
    {
        ...
        if ($feed->getProtocol() === 'sftp') {
            ...
            if ($privateKeyPath) {
                ...
                $privateKey->loadKey($this->driverFile->fileGetContents(self::FEED_KEY_FILE_PATH . $privateKeyPath));
                $password = $privateKey;
            }
