        ...
        foreach ($productCollection as $product) {
            ...
            if ($product->getTypeId() === "configurable") {
                ...
                foreach ($children as $child) {
                    ...
                    $sizeCode        = $child->getSku();
                    if($child->getCatalogRulePrice()){
                        $price       = $this->catalogHelper->getTaxPrice($child, $child->getCatalogRulePrice(), true);
                        $price= $this->convertPrice($price,$storeId);
                    }else{
                        $price       = $this->catalogHelper->getTaxPrice($child, $child->getPrice(), true);
                        $price       = $this->convertPrice($price,$storeId);
                    }

                    $size_ean        = $child->getEan();
                    $sourceDataBySku = $objectManager->create('\Magento\InventoryCatalogAdminUi\Model\GetSourceItemsDataBySku')->execute($sizeCode);

                    $arrQty = [];
                    foreach ($sourceDataBySku as $item){
                        $arrQty[] = intval($item['quantity']);
                        $sumQty = array_sum($arrQty);
                        $child->setData('size_qty',$sumQty);
                    }

                    $sizeStatus = $child->getStatus();

                    $child->setData('size',$size);
                    $child->setData('size_code',$sizeCode);
                    $child->setData('price',$price);
                    $child->setData('size_ean',$size_ean);
                    $child->setData('size_status',$sizeStatus);
