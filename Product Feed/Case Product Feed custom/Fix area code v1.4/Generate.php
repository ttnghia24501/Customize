...
class Generate extends Command
{
        ...
        if ($input->getArgument('ids')) {
            ...
            foreach ($feedIDs as $feedID) {
                try {
                    ...
                    $this->objectManager->create(Data::class)->generateAndDeliveryFeed($feed, 0, $type);
                    if (empty($this->objectManager->create(Data::class)->getEmailConfig('send_to'))) {
    ...
    {
        try {
            $this->objectManager->create(Data::class)->generateAndDeliveryFeed($feed, 0, 1);
        } catch (Exception $exception) {
