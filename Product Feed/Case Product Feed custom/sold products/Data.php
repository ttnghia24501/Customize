...
{% endfor %}';
    ...
    public function getProductsData($feed, $productAttributes = [], $productIds = [], $isSync = false)
    {
        ...
        foreach ($productCollection as $product) {
            ...
            try {
             ...
             if ($oriProduct->getAttributeText('material')) {
            ...
            $soldproduct = $this->objectManager->create('Magento\Reports\Model\ResourceModel\Product\Sold\Collection')
                ->addOrderedQty()->addAttributeToFilter('sku', $oriProduct->getSku())->setOrder('ordered_qty', 'desc')->getFirstItem();
            ...
            $product->setData('sold_number', $soldnumber);
    ...
    {
        if (in_array($field, ['manufacturer', 'material'])) {
    ...
    public function getReviewCollection($feed)
    {
        ...
        $collection         = $this->reviewFactory->create()->getCollection()->addRateVotes()
        ...
        foreach ($collection as $review) {
            ...
            $product = $this->productFactory->create()->setStoreId($storeId)->load($review->getEntityPkValue());
            ...
            $ratingVotesData = $review->getRatingVotes()->getData();
            $overallRating   = 0;
            if ($ratingVotesData) {
                foreach ($ratingVotesData as $vote) {
                    $overallRating += $vote['percent'] / 10;
                }
                $overallRating = $overallRating / count($ratingVotesData);
