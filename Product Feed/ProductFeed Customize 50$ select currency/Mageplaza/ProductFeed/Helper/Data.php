...
{% endfor %}';
    ...
    public function getProductPrice($product)
    {
        ...
        if ($product->getTypeId() == 'grouped') {
            ...
            foreach ($children as $child) {
                $childrenPrice[] = $child->getFinalPrice();
            }

            return count($childrenPrice) ? min($childrenPrice) : 0;
        }
    ...
    public function getProductUrl($product, $storeId)
    {
        ...
        $categories       = $this->categoryCollectionFactory->create()->addUrlRewriteToResult()
        ...
        if ($productURlConfig == ProductPath::SHORT || $productURlConfig == ProductPath::DEFAULT) {
            $category   = $categories->getFirstItem();
            $productUrl = $this->getUrl($product, $storeId, $category);
        } else {
     ...
     * @throws NoSuchEntityException
     */
    public function getUrl($product, $storeId, $category = null)
    {
        $useCatagoryConfig  = $this->getConfigValue('catalog/seo/product_use_categories', $storeId);
        $categoryUrlSuffix  = $this->getConfigValue('catalog/seo/category_url_suffix', $storeId);
        $storeBaseUrl       = $this->storeManager->getStore($storeId)->getBaseUrl(StoreUrlInterface::URL_TYPE_LINK);
        $productRequestPath = $product->getRequestPath();

        if ($useCatagoryConfig) {
            if (!$category->getRequestPath()) {
                $urlRewrite = $this->urlRewriteCollection->addStoreFilter($storeId)
                    ->addFieldToFilter('entity_type', 'category')
                    ->addFieldToFilter('entity_id', 4)
                    ->getFirstItem();
                $categoryRequestPath = str_replace($categoryUrlSuffix, '', $urlRewrite->getRequestPath() ?: '');
            } else {
                $categoryRequestPath = str_replace($categoryUrlSuffix, '', $category->getRequestPath() ?: '');
            }

            $productUrl          = $storeBaseUrl . $categoryRequestPath . '/' . ($productRequestPath !== null ? ltrim($productRequestPath,
                    '/') : '');
        } else {
            $productUrl = $storeBaseUrl . ($productRequestPath !== null ? ltrim($productRequestPath, '/') : '');
        }
    ...
    {
        return $this->getConfigGeneral('enabled')
            && $feed->getFileType() === 'xml'
    ...
    public function processProductField($field, $product)
    {
        ...
        if ($fielData = $product->getData($field)) {
            return $fielData;
        }

        if (($resource = $product->getResource()) && $productAttribute = $resource->getAttribute($field)) {
     ...
     * @throws Exception
     */
    public function requestData($url, $method = 'POST', $params = '', $isAccessToken = false)
    {
        ...
        if (isset($response['error'])) {
            $message = isset($response['error_description']) ? $response['error_description']
                : __('Something went wrong while syncing products. Please try again.');
    ...
    public function getReviewCollection($feed)
    {
        ...
        $collection         = $this->reviewCollectionFactory->create()
            ->addFieldToFilter('entity_pk_value', ['in' => $matchingProductIds]);
        $storeId            = $feed->getStoreId() ?: $this->storeManager->getDefaultStoreView()->getId();
        /** @var $review Review */
        foreach ($collection as $review) {
            ...
            $this->addUrlRewrite($product, $storeId);
            ...
            $reviewTimestamp = $this->timezone->date(new DateTime($review->getCreatedAt()))->format(DATE_ATOM);
            $review->setCreatedAt($reviewTimestamp);
            ...
            $overallRating   = 0;
            if ($votesCollection->getSize()) {
                foreach ($votesCollection as $vote) {
                    $overallRating += $vote['percent'] / 10;
                }
                $overallRating = $overallRating / $votesCollection->getSize();
    ...
    /**
     * Add URL rewrites to product
     *
     * @param $product
     * @param $storeId
     */
    public function addUrlRewrite($product, $storeId)
    {
        $filter = [
            'entity_type'      => 'product',
        ...
        foreach ($rewrites as $rewrite) {
            $product->setData('request_path', $rewrite->getRequestPath());
        }
    }
    ...
    protected function sendHttpRequest($feed)
    {
        ...
        $curl->write('POST', $requestUrl, '1.1', $header, $content);
