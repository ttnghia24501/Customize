...
class Data extends CoreHelper
{
    ...
    public function aroundGetProductsData($subject, callable $proceed, $feed, $productAttributes = [], $productIds = [], $isSync = false)
    {
        ...
        foreach ($productCollection as $product) {
            ...
            if ($oriProduct->getTypeId() !== 'configurable') {
                /** simple product saleable_qty*/
                $qtyBySku = $StockState->execute($oriProduct->getSku());
                $salableQty = $qtyBySku[0]['qty'];
                $product->setData('salable_qty', $salableQty);
            }
            if ($product->getTypeId() === "configurable") {
                ...
                foreach ($children as $child) {
                    $sourceDataBySku = $objectManager->create('\Magento\InventoryCatalogAdminUi\Model\GetSourceItemsDataBySku')->execute($child->getSku());

                    /** qty of configurable product = sum of children qty*/
                    foreach ($sourceDataBySku as $item) {
                        $arrQty []= intval($item['quantity']);
                        $sumQty = array_sum($arrQty);
                        $product->setData('qty', $sumQty);
                    }

                    /** saleable_qty of configurable product = sum of children saleable_qty*/
                    $salableQtyBySku = $StockState->execute($child->getSku());
                    $arrSalableQty[] = $salableQtyBySku[0]['qty'];
                    $sumSalableQty = array_sum($arrSalableQty);
                    $product->setData('salable_qty', $sumSalableQty);

                    /** max price*/
                    $arrPrice[] = $child->getPrice();
                    $maxPrice = max($arrPrice);
                    $product->setData('price', $maxPrice);

                    /** max special_price*/
                    $arrSpecialPrice[] = $child->getSpecialPrice();
                    $maxSpecialPrice = max($arrSpecialPrice);
                    $product->setData('special_price', $maxSpecialPrice);
                }
            }
