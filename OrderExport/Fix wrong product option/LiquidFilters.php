...
class LiquidFilters
{
    ...
    public function mpCorrect($subject, $fieldAround, $fieldSeparate)
    {
        ...
        if (is_array($subject) && array_key_exists('options', $subject)) {
            $result = [];
            foreach ($subject['options'] as $key => $value) {
                $label = $subject['options'][$key]['label'];
                $print_value = $subject['options'][$key]['print_value'];
                $result[] = $label . ': ' . $print_value;  
            }
            $subject = implode(',', $result);
        }
        if (is_array($subject) && !array_key_exists('options', $subject)) {
            $subject = '';
        }
     ...
     * @return mixed
     */
    public function minus($subject, $default)
    {
        if ($default) {
            $subject -= $default;
        }
